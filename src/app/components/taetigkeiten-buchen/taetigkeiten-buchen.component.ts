...
@Component({
  selector: 'app-taetigkeiten-buchen',
  imports: [
  ...
  templateUrl: './taetigkeiten-buchen.component.html',
  styleUrl: './taetigkeiten-buchen.component.scss',
...
// }
 private transformer = (node: TaetigkeitNode, level: number): FlatNode => {
  ...
  constructor(
    ...
    private taetigkeitenBuchenService: TatigkeitenBuchenService,
 ...
 loadData(personId: string) {
    ...
    this.taetigkeitenBuchenService.getPerson(
      personId,
...
const selectedBuchungspunkt = formValue.buchungspunkt as ApiProduktPositionBuchungspunkt;
  ...
  const startTime = Date.now();
  this.taetigkeitenBuchenService.createTaetigkeitsbuchung(
    dto,
    buchungspunktId,
    personId
  ).subscribe({
...
private performDelete(): void {
  ...
  getAlarmMinute(timeType: 'anmeldezeit' | 'abmeldezeit' | 'duration' = 'duration'): number {
    return this.timeUtilityService.getMinute(this.alarmForm, timeType);
  }
